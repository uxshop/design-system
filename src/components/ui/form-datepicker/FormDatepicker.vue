...
const emit = defineEmits<FormDatepickerEmits>();
...
const update = (value: string | null = null) => {
...
onMounted(() => {
  if (typeof props.config.singleMode === 'boolean') {
    singleMode.value = props.config.singleMode;
  }

  if (typeof props.config.numberOfMonths === 'number') {
    numberOfMonths.value = props.config.numberOfMonths;
  }

  if (typeof props.config.numberOfColumns === 'number') {
    numberOfColumns.value = props.config.numberOfColumns;
  }
  ...
  if (el) {
    const config: any = {
      ...
      setup(picker: easepick.Core) {
        picker.on('select', (e: CustomEvent) => {
          stopWatch();

          const { start, end, date } = e.detail as EasyPickDetails;
          const sqlFormat = 'YYYY-MM-DD';

          if (props.range && start && end) {
            update(`${start.format(sqlFormat)}--${end.format(sqlFormat)}`);
          } else if (date) {
            update(date.format(sqlFormat));
          }
...
const attrsToBind = computed(() => ({
  class: ['ui-form-datepicker', classList.value, attrs.class],
  style: attrs.style,
