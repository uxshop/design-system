...
(function () {
  ...
  Dom.prototype = {
    ...
    _queryContext: function (selector, context) {
      ...
      return context.nodeType !== 3 && typeof context.querySelectorAll === 'function'
  ...
  var RedactorApp = function (selector, options, args) {
    ...
    var nodes = Array.isArray(selector)
      ? selector
      ...
      for (var prop in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, prop)) {
          if (deep && Object.prototype.toString.call(obj[prop]) === '[object Object]')
            extended[prop] = $R.extend(true, extended[prop], obj[prop]);
  ...
  $R.AnimatePlay.prototype = {
    ...
    set: function (duration, iterate, timing) {
      ...
      while (len--) {
        ...
        if (iterate !== false || iterate === '')
          this.$el.css(this.prefixes[len] + 'animation-iteration-count', iterate);
  ...
  $R.add('service', 'selection', {
    ...
    is: function () {
      ...
      if (sel) {
        ...
        return (
          $R.dom(node).closest('.redactor-in-' + this.uuid).length !== 0 || node === this.editor.getElement().get()
        );
      }
  ...
  $R.add('class', 'toolbar.dropdown', {
    ...
    close: function (e, animate) {
      if (e) {
        ...
        if (
          this._isButton(e) ||
          $el.hasClass('redactor-dropdown-not-close') ||
          $el.hasClass('redactor-dropdown-item-disabled')
        ) {
          e.preventDefault();
          return;
        }
  ...
  $R.add('service', 'cleaner', {
    ...
    paste: function (html) {
      ...
      $wrapper.find('[data-redactor-tag]').each(
        function (node) {
          var $node = $R.dom(node);
      ...
      // remove empty inline
      $wrapper.find(this.opts.inlineTags.join(',')).each(
        function (node) {
  ...
  $R.add('service', 'insertion', {
    ...
    _sendNodes: function (nodes) {
      for (var i = 0; i < nodes.length; i++) {
        var el = nodes[i];
        var type =
          el.nodeType !== 3 && typeof el.getAttribute === 'function' ? el.getAttribute('data-redactor-type') : false;
        if (type) {
  ...
  $R.add('service', 'inline', {
    ...
    _revertTags: function (tag) {
      ...
      $editor.find('span.redactor-convertable-' + tag).each(
        function (node) {
    ...
    _revertToInlines: function () {
      ...
      // span convertable
      $editor.find('.redactor-convertable-apply').each(
        function (node) {
          var $node = $R.dom(node);
      ...
      // span unconvertable
      $editor.find('span.redactor-unconvertable-apply').each(
        function (node) {
          var $node = $R.dom(node);
    ...
    parse: function (html) {
      ...
      if (this.opts.autoparseImages && html.match(this.opts.regex.imageurl)) {
        ...
        for (var i = 0; i < imagesMatches.length; i++) {
          html = html.replace(
            imagesMatches[i],
            '<img class="redactor-autoparser-object" src="' + imagesMatches[i] + '">'
          );
        }
      }
    ...
    _format: function (enter) {
      ...
      if (current && current.nodeType === 3) {
        ...
        // video
          ...
          (content.match(this._convertToRegExp(this.opts.regex.youtube)) ||
            content.match(this._convertToRegExp(this.opts.regex.vimeo)))
        ) {
          var iframeStart = '<iframe width="500" height="281" src="';
          var iframeEnd = '" frameborder="0" allowfullscreen></iframe>';
          var str, re;
    ...
    _disableBrowsersEditing: function () {
      try {
        ...
        if (el.addEventListener)
          el.addEventListener('mscontrolselect', function (e) {
            e.preventDefault();
          });
        else
          el.attachEvent('oncontrolselect', function (e) {
            e.returnValue = false;
          });
      } catch (e) {}
    },
  ...
  $R.add('module', 'source', {
    ...
    show: function () {
      ...
      if (this.opts.source.hasOwnProperty('codemirror')) {
        ...
        var codemirrorSrc =
          typeof this.opts.source.codemirrorSrc !== 'undefined' ? this.opts.source.codemirrorSrc : CodeMirror;
  ...
  $R.add('module', 'toolbar', {
    ...
    _buildImageButton: function () {
      if (!this.opts.imageUpload && !this.opts.imageManagerJson)
        this.utils.removeFromArrayByValue(this.buttons, 'image');
    },
  ...
  $R.add('module', 'link', {
    ...
    _insertSingle: function (links, data) {
      ...
      if (
        links.length === 1 &&
        (links[0].textContext === this.selection.getText() || (inline && inline.tagName === 'A'))
      ) {
  ...
  $R.add('module', 'modal', {
    ...
    _needToClose: function (el) {
      var $target = $R.dom(el);
      if (
        $target.attr('data-action') === 'close' ||
        this.$modal.isCloseNode(el) ||
        $target.closest('.redactor-modal').length === 0
      ) {
        return true;
      }
  ...
  $R.add('class', 'input.enter', {
    ...
    _isExit: function (e) {
      ...
      // li
        ...
        data.isComponent() &&
        data.isComponentActive() &&
        !data.isFigcaption() &&
        !data.isComponentEditable()
      ) {
  ...
  $R.add('class', 'input.paste', {
    ...
    _init: function (e) {
      ...
      if (this.isRawCode || !clipboard) {
        ...
        var html = this._isPlainText(clipboard)
          ? this.cleaner.encodeEntities(clipboard.getData('text/plain'))
    ...
    _insert: function (e, html) {
      ...
      if (this.isRawCode) {
        ...
        nodes = this.dropPasted
          ? this.insertion.insertToPoint(e, html, this.pointInserted)
    ...
    _insertSingle: function (response, e) {
      for (var key in response) {
        if (typeof response[key] === 'object') {
          ...
          var inserted = e
            ? this.insertion.insertToPoint(e, $img, false, false)
            : this.insertion.insertHtml($img, false);
  ...
  $R.add('class', 'image.resize', {
    ...
    _getResizableBoxWidth: function () {
      ...
      return (
        width - parseInt(this.$resizableBox.css('padding-left')) - parseInt(this.$resizableBox.css('padding-right'))
      );
    },
  });
  $R.add('module', 'file', {
    ...
    _createFileAndStore: function (item) {
      ...
      var title =
        !this.opts.fileAttachment && modalFormData && modalFormData.title !== ''
          ? modalFormData.title
          : this._truncateUrl(name);
  ...
  $R.add('module', 'list', {
    ...
    toggle: function (type) {
      ...
      nodes =
        nodes.length !== 0 && this._isUnformat(type, nodes) ? this._unformat(type, nodes) : this._format(type, nodes);
        ...
        // 4) middle
          ...
          $items.each(function (node) {
            if (ci) {
              var $node = $R.dom(node);
              if (
                $node.closest('.redactor-split-item').length === 0 &&
                ($parent === false || $node.closest($parent).length === 0)
              ) {
