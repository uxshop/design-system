...
// mcp/tools/get-component-bundle.ts
function registerGetComponentBundle(server) {
  server.registerTool(
...
function registerSearchComponents(server) {
  server.registerTool(
    'search_components',
    {
      description:
...
function registerGetDesignTokens(server) {
  server.registerTool(
    'get_design_tokens',
    {
      description:
...
function registerTokensResources(server) {
...
function registerCatalogResource(server) {
  server.registerResource(
    'component-catalog',
...
function registerTypesResource(server) {
  server.registerResource(
    'design-system-types',
    'design-system://types',
    {
...
function registerUseComponentPrompt(server) {
...
function registerCreateComponentPrompt(server) {
  server.registerPrompt(
    'create-component',
    {
...
function registerApplyTokensPrompt(server) {
  server.registerPrompt(
    'apply-tokens',
    {
...
// mcp/start-http.ts
var basePort = Number(process.env.MCP_HTTP_PORT ?? 3100);
var maxPortAttempts = 10;
var sessionTtlMs = 30 * 60 * 1e3;
...
async function handleMcpRequest(req, res) {
  ...
  const parsedBody = req.method === 'POST' ? await readBody(req) : void 0;
...
function startWithFallback(port, attempt = 1) {
  if (attempt > maxPortAttempts) {
    console.error(`Nenhuma porta dispon\xEDvel entre ${basePort} e ${basePort + maxPortAttempts - 1}. Abortando.`);
    process.exit(1);
  }
